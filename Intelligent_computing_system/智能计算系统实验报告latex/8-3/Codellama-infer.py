...
input_ids = input_ids.int().to("mlu")
attention_mask = torch.ones_like(input_ids)
#TODO： 使用模型生成文本序列，给定输入张量 input_ids，限制最大生成标记数为 240，启用缓存机制
output = model.generate(input_ids, attention_mask = attention_mask, max_length=240, use_cache=True, pad_token_id=tokenizer.eos_token_id)
...
latency = t2 - t1
#TODO:计算输出的token数量
output_token_num = output.shape[1]
