...
def get_gpu_memory(max_gpus=None):
    ...
    # 存储每个GPU的可用内存信息
    gpu_memory = []
    # 获取MLU设备的数量：如果未指定最大GPU数，则使用所有可用设备；否则取两者中较小值
    available_devices = torch_mlu.device_count()
    num_gpus = (
        available_devices if max_gpus is None else min(max_gpus, available_devices)
    )
    ...
    for gpu_id in range(num_gpus):
        ...
        # 获取当前MLU设备
        device = torch.device(f"mlu:{gpu_id}")
        # 获取当前MLU设备的属性
        gpu_properties = torch_mlu.get_device_properties(gpu_id)
        # 获取总内存，单位转换为GB
        total_memory = gpu_properties.total_memory / (1024**3)
        # 获取已分配内存，单位转换为GB
        allocated_memory = torch_mlu.memory_allocated(gpu_id) / (1024**3)
        # 计算可用内存，单位转换为GB
